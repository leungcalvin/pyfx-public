...
class VeryBasicBBData:
    ...
    def __getitem__(self, key):
        if key == "tiedbeam_baseband":
            return self.data
        if key == "time0":
            return {"ctime": self.ctime, "ctime_offset": self.ctime_offset}
...
def dont_test_autocorr_sim():  # deprecated test, renamed from test_autocorr_sim
    ...
    data = (
        np.random.uniform(-5e6, 5e6, (nfreq, npol * npointing, ntime))
        + 1j * np.random.uniform(-5e6, 5e6, (nfreq, npol * npointing, ntime))
    ) * 0
    amplitude_00 = 8 + 0j
    amplitude_11 = 50 + 0j
    ...
    vectorized_autocorr = autocorr_core(
        DM=1.2,
        bbdata_a=bbdata_a,
        t_a=t_a,
        window=window,
    ...
    vectorized_autocorr_zp = autocorr_core(
        DM=1.2, bbdata_a=bbdata_a, t_a=t_a, window=window, R=R, max_lag=max_lag, zp=True
    )
    assert np.isclose(
        vectorized_autocorr, vectorized_autocorr_zp
    ).all(), "autocorr zp does not work"
...
def test_continuum_calibrator():
    ...
    duration_sec = int(2.56e-6 * (chime_bbdata.ntime + sweep_duration_frames) + 50)
    nscan = 1
    npointing = 1
    max_lag = 100
    ...
    buffer_seconds = 1
    good_indices = [
        np.where(np.max(chime_bbdata["tiedbeam_baseband"][:, 0, :], axis=-1) != 0.0)[0]
    ]  # these have not been waterfall filled
    real_times = Time(
...
def test_pulsar_core():
    ...
    telescope_names = ["chime", "kko"]
    chime_file = (
        "/arc/projects/chime_frb/pyfx_test_files/304050301_target_B0355+54_chime.h5"
    )
    kko_file = (
        "/arc/projects/chime_frb/pyfx_test_files/304050301_target_B0355+54_kko.h5"
    )
    chime_bbdata = BBData.from_file(chime_file)
    out_bbdata = BBData.from_file(kko_file)
    fill_waterfall(chime_bbdata, write=True)
    ...
    cross = crosscorr_core(
        ...
        sample_rate=2.56,
        max_lag=max_lag,
        n_pol=2,  # ref_frame=0,#ref_frame=0,
        weight=weight,
    )
    ...
    assert (
        snrs[0, 0] >= 39
    ), f"fringe signal to noise is below expected value in 0,0 pol, expected (39,30), got {snrs[0,0]}"
    assert (
        snrs[1, 1] >= 30
    ), f"fringe signal to noise is below expected value in 1,1 pol,expected (39,30), got {snrs[1,1]}"
    assert np.isclose(
        delays[0, 0], -0.21765625, rtol=1e-04
    ), f"delays[0,0] wrong! Delays evaluated to be {delays}"  # should be good to sub nanosecond
    assert np.isclose(
        delays[1, 1], -0.21578125, rtol=1e-04
    ), f"delays[1,1] wrong! Delays evaluated to be {delays}"  # should be good to sub nanosecond
...
def test_pulsar_pycalc_corrjob():
    ...
    for ifreq in range(len(cross)):
        ...
        if val_11 > cutoff_11:
            cross[ifreq, 0, 1, 1, :, 0] *= 0
    cross_pycalc = cross
    peaklags = extract_frame_delay(cross_pycalc[:, 0, :, :, :, 0])
    peaklag_00 = peaklags[0]
    peaklag_11 = peaklags[1]
    ...
    assert np.isclose(
        delays_pycalc[0, 0], -0.21765625, atol=1e-03
    ), f"delays[0,0] wrong! Got {delays_pycalc[0,0]}!"
    assert np.isclose(
        delays_pycalc[1, 1], -0.21578125, atol=1e-03
    ), f"delays[1,1] wrong! Got {delays_pycalc[1,1]}!"

    assert (
        snrs_pycalc[0, 0] >= 37
    ), f"fringe signal to noise is below expected value in 0,0 pol, got {snrs_pycalc[0,0]}"
    assert (
        snrs_pycalc[1, 1] >= 27
    ), f"fringe signal to noise is below expected value in 1,1 pol, got {snrs_pycalc[1,1]}"
