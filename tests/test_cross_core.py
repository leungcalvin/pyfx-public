...
def test_autocorr_vectorized_1():
    ...
    npol=2
    nscan=1
    t_a=np.random.randint(0,10,size=(nfreq,1,nscan))*0+10
    ...
    R=np.random.uniform(.5,1,(nfreq,1,nscan))
    max_lag=10
    window=np.ones((nfreq,1,nscan))*40
    ...
    for i in range(nscan):
        assert (np.imag(vectorized_autocorr[...,i,0,0,0,0])==0.0).all()
        assert (np.imag(vectorized_autocorr[...,i,1,1,0,0])==0.0).all()
...
def test_autocorr():
    ...
    for iifreq in range(nfreq):
        for scan in range(nscan):
            data[iifreq,scan*2,t_a[iifreq,0,scan]+1]=2+2j #peakval 8
            data[iifreq,scan*2+1,t_a[iifreq,0,scan]+1]=5+5j #peakval 50
    ...
    for i in range(nscan):
        ...
        assert (vectorized_autocorr[...,i,1,1,0,0]==amplitude_11).all()==True
