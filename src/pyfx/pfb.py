...
def bb_covar_kernel(delay_samples, frame_lag, freq_ind, channelization):
    ...
    window_auto_at_lags = pfb_window_auto_corr(delay_relative_to_frame_delay,
            channelization)

    phase_fac = delay_phase_factor(delay_samples, freq_ind, channelization = channelization)
...
def pfb_window_auto_corr(delay_samples, channelization):
    ...
    nfreq, lblock, ntap = channelization['nchan'],channelization['lblock'],channelization['ntap']
    pfb_window = sinc_hamming(channelization)
    len_window = len(pfb_window)
    window_auto_corr = signal.correlate(pfb_window, pfb_window,
            mode='full')
    ...
    if False:
        window_auto_corr_lags_samples = correlation_lags(len(pfb_window), len(pfb_window))
        window_auto_corr_lags_samples = np.roll(window_auto_corr_lags_samples, -(len(pfb_window) - 1))
        plt.figure()
        plt.plot(window_auto_corr_lags_samples, window_auto_corr.real, ls='', marker='.')
...
def calculate_inv_noise_covar_kernel(channelization):
    ...
    inv_noise_covar_kernel = ifft(1. / fft(noise_covar_kernel)).real # Kiyo math, don't understand
