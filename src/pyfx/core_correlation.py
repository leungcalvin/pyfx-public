...
def autocorr_core(
    ...
    bbdata_a["tiedbeam_baseband"][:] = np.nan_to_num(
        bbdata_a["tiedbeam_baseband"][:], nan=0, posinf=0, neginf=0
    )
...
def crosscorr_core(
    ...
    bbdata_b["tiedbeam_baseband"][:] = np.nan_to_num(
        bbdata_b["tiedbeam_baseband"][:], nan=0, posinf=0, neginf=0
    )
    ...
    for kkpointing in range(n_pointings):
        for jjscan in range(n_scan):
            ...
            t0_a_offset = bbdata_a["time0"]["ctime_offset"][:] + t_a_indices * (
                sample_rate * 1e-6
            ...
            delta_ctime_offset = (
                t0_a_offset - t0_a_offset[0]
            )  # difference between reference start time and nth freqeucny start time
            ...
            if fast:  # for the impatient
                ...
                dt_vals0 = (
                    ref_start_time - pycalc_results.times[0]
                ).sec  # should always be <1s.
                delays_flattened = pycalc_results.delays_dt(
                    dt_vals0 + dt_vals.flatten()
                )
                geodelays_flattened = (
                    delays_flattened[:, 0, index_B, :]
                    - delays_flattened[:, 0, index_A, :]
                )  # units of seconds
                geodelays = (
                    geodelays_flattened.reshape(dt_vals.shape) * 1e6
                )  # microseconds #nfreq,nframe
            else:  # for the paranoid
                t0_a_offset = bbdata_a["time0"]["ctime_offset"][
                    :
                ]  # array of length 1024
                start_times = Time(
                    t0_a,
                ...
                for i in range(n_freq):
                    ...
                    query_times = start_times[i] + sample_rate * 1e-6 * un.s * (
                        t_a_indices[i] + np.arange(wij)
                    )
                    delays = pycalc_results.interpolate_delays(query_times)
                    geodelays[i, :] = (
                        delays[:, 0, index_B, 0] - delays[:, 0, index_A, 0]
                    ) * 1e6
            ...
            if len(np.unique(r_jjscan)) == 1:
                ...
                if start == stop:
                    ...
                    for pol_0 in range(n_pol):
                        for pol_1 in range(n_pol):
                            assert not np.isnan(
                                np.min(scan_a_cd[:, pol_0, start:stop].flatten())
                ...
                for freq in range(len(r_jjscan)):
                    ...
                    if start == stop:
                        ...
                        for pol_0 in range(n_pol):
                            for pol_1 in range(n_pol):
                                assert not np.isnan(
                                    np.min(scan_a_cd[freq, pol_0, start:stop].flatten())
                                ), "Scan parameters have been poorly defined for telescope A. Please ensure there are no nans in the baseband data"
                                assert not np.isnan(
                                    np.min(
...
def get_aligned_scans(
    ...
    a_shape = list(
        bbdata_a["tiedbeam_baseband"][:, npointing : npointing + n_pol, :].shape
    )
    a_shape[-1] = wij
    ...
    if len(np.unique(t_a_index)) == 1 and not zp:
        aligned_a[:, ...] = bbdata_a["tiedbeam_baseband"][
            :, npointing : npointing + n_pol, t_a_index[0] : t_a_index[0] + wij
        ]

    elif len(np.unique(t_a_index)) > 1 and not zp:
        for i in range(len(t_a_index)):
            aligned_a[i, ...] = bbdata_a["tiedbeam_baseband"][
                i, npointing : npointing + n_pol, t_a_index[i] : t_a_index[i] + wij
            ]
    elif zp:
        for i in range(len(t_a_index)):
            for j in range(n_pol):
                aligned_a[i, j, :] = getitem_zp1d(
                    bbdata_a["tiedbeam_baseband"][i, j, :],
                    t_a_index[i],
                    t_a_index[i] + wij,
                )
    ...
    delta_A_B = (t_b - t_a).to_value(
        "sec"
    )  # this is actually not that time consuming for 1024 frequencies

    int_delay = np.array(
        [
            int(np.round((timeb * 1e-6 - delta) / (sample_rate * 1e-6)))
    ...
    if not zp:
        ...
        new_wij = np.array(
            [
                np.min([wij - pad, bbdata_b.ntime - start])
        ...
        for i in range(len(t_a_index)):
            for j in range(n_pol):
                aligned_b[i, j, :] = getitem_zp1d(
                    bbdata_b["tiedbeam_baseband"][i, j, :],
    ...
    sub_frame_tau = np.array(
        [
            tau[i, :wij] - time_b * 1e6
            for time_b, i in zip(time_we_have_at_b, range(len(tau)))
        ]
