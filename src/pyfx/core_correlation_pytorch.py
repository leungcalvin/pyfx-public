        ...
        for iipol in range(n_pol):
            for jjpol in range(n_pol):
                for jjscan in range(n_scan):
                    ...
                    if (type(r_jjscan) == float or type(r_jjscan) == int) and len(
                        np.unique(t_a_indices)
                    ) == 1:
                        r_ij = r_jjscan
                        start = (
                            int((window_jjscan - window_jjscan * r_ij) // 2)
                            + t_a_indices[0]
                        )
                        stop = (
                            int((window_jjscan + window_jjscan * r_ij) // 2)
                        ...
                        auto_vis[:, pointing, iipol, jjpol, jjscan, :] = np.concatenate(
                        ...
                        for iifreq, r_ij in enumerate(r_jjscan):
                            ...
                            auto_vis[iifreq, pointing, iipol, jjpol, jjscan, :] = (
                                torch.concat((_vis[: max_lag + 1], _vis[-max_lag:]))
                            )
    return auto_vis
...
def crosscorr_core_vectorized_gpu(
    ...
    ** also currently only tested for steady sources with constant R.
    ...
    ctime_offset_diff = (
        bbdata_A["time0"]["ctime_offset"] - bbdata_B["time0"]["ctime_offset"]
    )
    ...
    for pointing in range(n_pointings):
        ...
        for jjscan in range(n_scan):
            ...
            t_a_indices = t_a[:, jjscan]  # array of length 1024
            t0_a = bbdata_A["time0"]["ctime"][:] + t_a_indices * (
                sample_rate * 1e-6
            ...
            dt_vals = (
                sample_rate
                * 1e-6
                * (t_a_indices[:, np.newaxis] + 1 + np.arange(window_jjscan))
            )
                    ...
                    for pol_0 in range(n_pol):
                        for pol_1 in range(n_pol):
                            if pol_0 == pol_1:
                                ...
                                cross[:, pointing, pol_0, pol_1, jjscan, :] = (
                                    torch.concat(
...
def get_aligned_scans_gpu(
    ...
    if len(np.unique(t_a_index)) == 1:
        aligned_a[:, ...] = baseband_a[:, ..., t_a_index[0] : t_a_index[0] + wij]
    else:
        for i in range(len(t_a_index)):
            aligned_a[i, ...] = bbdata_A["tiedbeam_baseband"][
                i, ..., t_a_index[i] : t_a_index[i] + wij
            ]
    end = time.time()
    ...
    start = time.time()
    aligned_b = torch.zeros(baseband_a.shape, dtype=baseband_a.dtype)
    ...
    if correction_factor.any() > 2:
        ...
        print(
            "warning: based on specified start time and scan length, over half the data is missing from telescope XX."
        )
    ...
    for i in range(len(pad_index_b)):
        aligned_b[i, ..., pad_index_b[i] : pad_index_b[i] + new_wij[i]] = (
            baseband_b[
                i,
                ...,
                start_index_we_have_at_b[i] : start_index_we_have_at_b[i] + new_wij[i],
            ]
            * correction_factor[i]
        )
    end = time.time()
...
def frac_samp_shift_gpu(
    ...
    f = f.to(device)
    transfer_func = torch.exp(
        intra_channel_sign
        * 2j
        * np.pi
        * f[np.newaxis, :]
        * torch.median(sub_frame_tau, dim=-1).values[:, np.newaxis]
    )  # apply dphi/dfreq
    del f  # free up vram
    output = (
        torch.fft.ifft(torch.fft.fft(data) * transfer_func[:, np.newaxis])
        * (
            torch.exp(
                complex_conjugate_convention
                * 2j
