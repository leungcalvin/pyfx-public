...
#### NEW POSITION, as of Sept '23 ######
kko = ac.EarthLocation.from_geocentric(
    x = (-2111738.254-19.951) * un.m,
    y = (-3581458.222+7.155) * un.m, 
    z = (4821611.987-0.556) * un.m
) 
kko.info.name = 'kko'
...
def tels_to_ndarray(tels_list):
    ...
    """
    if type(tels_list) not in [list, np.ndarray]: # unfortunately, checking __iter__ and __len__ do not work
        tels_list = [tels_list]
    tels_array = np.empty(
                        len(tels_list),
                        dtype = TEL_DTYPE)
...
def tels_to_astropy(tels_array):
    ...
    tels_list = []
    if type(tels_array) not in [list, np.ndarray]: # unfortunately, checking __iter__ and __len__ do not work
