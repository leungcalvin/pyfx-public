...
def signal_to_noise_weight_correlator(
    w1,
    w2,
    signal_kernel,
    max_lag=None,
...
def subframe_signal_to_noise_search_correlator(
    data_bb_1, data_bb_2, channelization=CHANNELIZATION, full_output=False
):
    ...
    for ii, subframe_delay in enumerate(channelization["search_lags"]):
        ...
        w1_sivw = convolve_bb_time_kernel(
            w1_ivw, kernel=channelization["SIG_SEARCH_KERNEL"][ii, :]
        )
        this_cross_corr_func = fft_corr(w1_sivw, w2_ivw)
        this_frame_lags = (
            subframe_delay / channelization["lblock"]
            + fftfreq(w1_sivw.shape[-1]) * w1_sivw.shape[-1]
        )
        ...
        assert (
            w1_sivw.shape[0] == channelization["nchan"]
        ), "Wrong number of channels; search correlator requires 1024 channels at present."

        if ii == 0:
            # Initialize global output arrays
            nframe_lags = len(this_frame_lags)
            frame_lags = np.zeros((n_search_lags, nframe_lags), this_frame_lags.dtype)
            cross_corr_func = np.zeros(
                (
                    this_cross_corr_func.shape[0],
                    n_search_lags,
                    this_cross_corr_func.shape[-1],
                ),
    ...
    frame_lags = (
        ...
        .swapaxes(0, 1)
        .reshape((nframe_lags * n_search_lags))
    )
    cross_corr_func = (
        fftshift(cross_corr_func, axes=(1, 2))
        .swapaxes(1, 2)
        .reshape((nchan, nframe_lags * n_search_lags))
    )
    ...
    cross_corr_func = np.roll(
        fftshift(cross_corr_func, axes=-1), -n_search_lags // 2, axis=-1
    )
    if full_output:
        return cross_corr_func, frame_lags
...
def max_lag_slice(data: np.ndarray, max_lag: int, lag_axis: int = -1) -> np.ndarray:
