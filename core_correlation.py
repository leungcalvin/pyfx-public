...
def correlation_core(DM, bbdata_A, bbdata_B, T_A, Window, R, calc_results,index_A=0,index_B=1):
    ...
    shape_output_data=(bbdata_A.nfreq,len(time_chunks),2, 2,time_delay_num)
    ...
    for j in time_chunks: 
        ...
        for pol_1 in pols:  
            autos_A[:, j, pol_1, pol_1, :] = ifft(fft(bbdata_A_signal_on[pol_1],bbdata_A_signal_on[pol_1],axis=-1)
            autos_B[:, j, pol_1, pol_1, :] = ifft(fft(bbdata_B_signal_on[pol_1],bbdata_B_signal_on[pol_1],axis=-1)
            for pol_2 in pols:
                autos_cross[:, j, pol_1, pol_2, :] = ifft(fft(bbdata_A_signal_on[pol_1],bbdata_B_signal_on[pol_2],axis=-1)
